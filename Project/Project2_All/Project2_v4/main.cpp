...
V3:
 - Code Cleanup
 - Display Cleanup
 - Increased Max Hand Size to 21 (max possible hands if you are reallllyyy unluckly)
*/
...
int main(int argc, char** argv) {
    ...
    std::cout << "========== CLI BLACKJACK ==========" << std::endl << std::endl;
...
void playRound(short rndNum, std::vector<player> &players){
                ...
                switch (res) {
                    ...
                    case 2:
                        ...
                        if (players.at(i).isAi == false) {
                            std::cout << "Human Player Busted, Game over!" << std::endl;
                            return;
                        }
    ...
    float avg = (float) getScore(players.at(winIdx).hand) / 21;
...
void displayHand(std::string name, short (&hand)[21]) {
...
void aiChoose(bool &stay, short (&hand)[21]){
